import type { Job, Person } from '@app/types/payload'
import type { MigrationFunction, tmdbPerson } from '../types'
import { findOrCreateDoc } from '@app/util/payload/findOrCreateDoc'
import { updateDocInAllLocales } from '@app/util/payload/updateDocInAllLocales'
...
export const migrateCredits: MigrationFunction = async ({ payload, movie, warnings }) => {
  ...
  const data = await fetchData('credits', movie.tmdbId)
