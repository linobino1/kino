    ...
    posts.forEach(async (post) => {
      if (post.link && typeof post.link === 'string') {
        if (post.link.startsWith('http') || post.link.startsWith('www')) {
          await db.collection('posts').updateOne({ _id: post._id }, {
            $set: {
              link: {
                type: 'external',
                url: post.link,
    ...
    posts.forEach(async (post) => {
      if (post.link) {
        if (post.link.type === 'external') {
          await db.collection('posts').updateOne({ _id: post._id }, { $set: { link: post.link.url, } });
        } else { 
          await db.collection('posts').updateOne({ _id: post._id }, { $unset: { link: '', } });
        }
      }
    });
  }
